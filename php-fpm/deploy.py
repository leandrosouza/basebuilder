...
class Manager(object):
    ...
    def install(self):
        ...
        if self.configuration.get('composer', None) is not None:
    ...
    def install_composer(self):
        if os.path.isfile(os.path.join(self.application.get('directory'), 'composer.json')):
            ...
            composer_conf = self.configuration.get('composer', None)
            dev_dependencies = ''

            if composer_conf is not None
                dev_dependencies = '--no-dev' if composer_conf.get('dev', False) else dev_dependencies

            if os.system('cd %s && %s install %s' % (self.application.get('directory'), composer_phar, dev_dependencies)) != 0:
